...
public:
	...
	void write(float f) override {
		...
		f = (float) floorNum / 1000.0f;
		stringstream s;
		s << f;
		string text = s.str();
		if (send(client_fd, text.c_str(), text.length(), 0) < 0)
			throw "error writing float";
	}
...
public:
	 ...
	 */
	AnomalyDetectorCommand(DefaultIO* dio, AnomalyCommandInfo* hAD) : Command(dio),
	                                                                  commandInfo(hAD) {}
};
...
public:
	...
	AlgoSettingsCommand(DefaultIO* dio, AnomalyCommandInfo* hAD) : AnomalyDetectorCommand(dio,
	                                                                                      hAD) {
...
public:
	...
	DetectAnomaliesComand(DefaultIO* dio, AnomalyCommandInfo* hAD) : AnomalyDetectorCommand(dio,
	                                                                                        hAD) {
...
public:
	...
	UplaodAndAnalyzeCommand(DefaultIO* dio, AnomalyCommandInfo* hAD) : AnomalyDetectorCommand(dio,
	                                                                                          hAD) {
	...
	void execute() override {
		...
		while (line != "done") {
			...
			while (itA != timeStepAnomalies.end()) {
				if (betweenTwoNums(itA->first, start, end) ||
				    betweenTwoNums(itA->second, start, end) ||
				    betweenTwoNums(start, itA->first, itA->second) ||
				    betweenTwoNums(start, itA->first, itA->second)) {
					countInLoop = true;
		...
		dio->write(countTP / countP);
		dio->write("\nFalse Positive Rate: ");
		dio->write(countFP / countN);
		dio->write("\n");
	}
